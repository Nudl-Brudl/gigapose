...
from src.utils.file_management import (get_cam_K_from_yaml, 
                                       write_detector_yaml, 
                                       copy_and_rename)
...
stoiber_recording_dir = os.path.join(stoiber_dir, "my_recording")
stoiber_data_dir = os.path.join(stoiber_dir, "data", "pen_paper_demo")

detector_yaml_path = os.path.join(stoiber_data_dir, obj_name + "_detector.yaml")

# XMem paths
xmem_workspace = os.path.join(root_dir, "workspace")
xmem_path = os.path.join(root_dir, "..", "Koenig_Johannes", "XMem", "interactive_demo.py")
...
for idx in range(poses_est.shape[0]):
    boxed_img = draw_xyz_axis(rgb_orig, poses_est[idx], scale=20, K=camera_params)
    cv2.imshow(f"Pose number {idx+1}", boxed_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

print(prediction_list[0].pred_poses[0, 0, :3, :3])
print(prediction_list[0].pred_poses[0, 0, :3, 3])
print(camera_params)
logger.info("Inference Done")


pose_stoiber = poses_est[0]

write_detector_yaml(detector_yaml_path, pose_stoiber)
