...
def draw_xyz_axis(color, 
                  ...
                  is_input_rgb=False):
    '''
    Draws a coordinate system onto the given color image
    @color: BGR
    '''

    # Make sure that image is BGR
    if is_input_rgb:
        color = cv2.cvtColor(color,cv2.COLOR_RGB2BGR)

    # Axis homogeneous vectors
    xx = np.array([1,0,0,1]).astype(float)
    yy = np.array([0,1,0,1]).astype(float)
    zz = np.array([0,0,1,1]).astype(float)

    # Scale the axis vectors
    xx[:3] = xx[:3]*scale
    yy[:3] = yy[:3]*scale
    zz[:3] = zz[:3]*scale

    # Project vectors to 2d image
...
def draw_posed_3d_box(K, img, ob_in_cam, bbox, line_color=(0,255,0), linewidth=2):
  ...
  Draws a bounding box onto given image
  @bbox: (2,3) min/max
  @line_color: RGB
  '''
